...
미해결이던 "빌드가 이전 산출물을 재사용" 문제의 원인을 찾았다.
이 PC 의 Node 24.13.0 은 **경로에 한글이 있으면 파일 · 디렉터리 삭제가 프로세스째 죽는다**
(`0xC0000409`, `d:\tmp\한글only` 로 재현, 같은 위치 ASCII 경로는 정상).
vite 가 `dist` 를 비우다 죽어 낡은 번들이 남았고, 셸 파이프가 종료 코드를 가려 성공처럼 보였다.

대응
- `prebuild` 에서 OS 의 `rmdir` 로 `dist` 삭제 (`scripts/prebuild-clean.mjs`)
- vite `emptyOutDir` 끔 — 지우는 일은 prebuild 가 전담
- `scripts/verify-build.mjs` 가 빌드 후 산출물이 이번 실행에서 만들어졌는지 검증
- Playwright 포트를 4173 → 4319 로 옮기고 `reuseExistingServer` 를 껐다.
  4173 은 이 PC 의 다른 프로젝트 개발 서버가 쓰고 있어 엉뚱한 앱을 테스트할 위험이 있었다

검증: dist 가 남아 있는 상태로 연속 3회 빌드 성공, e2e 10개 통과.

## 남은 사항

- `docs/ISSUE_REPORT_KO.md` 의 open 이슈 4건: 자사 API 명세 수령(#4), 스트리밍 목업의 실연동 교체(#8), 점수 가중치 실데이터 튜닝(#11), Node 한글 경로 삭제 크래시(#28 — Node 최신 LTS 로 올리거나 작업 경로를 ASCII 로 옮기면 해결)
